...
export const HomeForm = () => {
  ...
  function handleSelectOption(value: number) {
    setSelectedOptions(prevState => prevState.map(option => {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const encryptedInstallment = encryptData(
  ...
  return (
    <form onSubmit={handleSubmit} className='my-4 flex flex-col items-center p-6 w-full'>
      <h1 className="text-center text-2xl font-medium max-w-[400px]">
        Faça sua simulação em menos de 5 minutos!
      </h1>

      <Card>
        <h2 className="text-[22px] text-center font-medium">Quanto você deseja pegar emprestado?</h2>
        ...
        <p className="text-sm mt-2 text-details font-medium">O valor pode ser de R$ 150,00 à R$ 50.000,00.</p>
      </Card>

      <Card>
        <h2 className="text-[22px] text-center font-medium">Em quantas vezes você deseja pagar
        </h2>
        ...
        <p className="mt-6 text-lg text-details">
          Sua parcela mensal será de <strong className="text-primary">{getInstallmentValue}</strong>
        </p>
      </Card>

      <Button type="submit" className="px-14 mt-6">Continuar</Button>
    </form>
  )
}
