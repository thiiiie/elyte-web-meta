...
const formSchema = z.object({
  monthlyEarnings: z.string({ required_error: 'Digite sua renda mensal' }).min(1, 'Digite sua renda mensal'),
  payday: z.string({ required_error: 'Digite o dia do seu pagamento' }).min(1, 'Digite o dia do seu pagamento').max(31, 'Digite um dia válido'),
  ocupation: z.string({ required_error: 'Selecione sua ocupação' }).min(1, 'Selecione sua ocupação'),
  education: z.string({ required_error: 'Selecione sua escolaridade' }).min(1, 'Selecione sua escolaridade'),
})
...
export const LoanInfoForm = () => {
  ...
  return (
    <form className="flex flex-col gap-4 mt-2" onSubmit={handleSubmit(onSubmit)}>
      ...
      <Button type="submit" className="w-fit px-10">Continuar</Button>
    </form>
  )
}
